...
export const contextOptsStack = [];
function normalizeHistory(history2, base) {
  let normalizedHistory = {};
  if (base)
    normalizedHistory.basename = base;
  if (history2) {
    if (typeof history2 === "string") {
      normalizedHistory.type = history2;
    } else {
      normalizedHistory = history2;
    }
  }
...
export function genBootstrap(oldRender) {
  return async (props) => {
  ...
  return async (props) => {
    if (typeof props !== "undefined") {
      ...
      const clientRenderOpts = {
        ...
        rootElement: props.container?.querySelector(`#${mountElementId}`) || document.getElementById(mountElementId),
        basename: props.base,
...
export function genUnmount(mountElementId) {
  return async (props) => {
    ...
    if (typeof root?.unmount === "function") {
      ...
      const container = props?.container ? props.container.querySelector(`#${mountElementId}`) : document.getElementById(mountElementId);
      if (container) {
